...
class DefaultReactiveMessageReaderFactoryTests {
	...
	void createReaderWithDefaults() {
		...
		ReactiveMessageReader<String> reader = readerFactory.createReader(schema, null);
	...
	@Test
	void createReaderOverrideDefaults() {
		MutableReactiveMessageReaderSpec spec = new MutableReactiveMessageReaderSpec();
		spec.setReaderName("test-reader");
		DefaultReactivePulsarReaderFactory<String> readerFactory = new DefaultReactivePulsarReaderFactory<>(
				AdaptedReactivePulsarClientFactory.create((PulsarClient) null), spec);
		MutableReactiveMessageReaderSpec newSpec = new MutableReactiveMessageReaderSpec();
		newSpec.setReaderName("new-test-reader");

		ReactiveMessageReader<String> reader = readerFactory.createReader(schema, newSpec);
