...
If you throw any other exception, the framework will nack the entire batch and log a warning, rather than applying the per-message retry and recovery logic provided by the error handler.
