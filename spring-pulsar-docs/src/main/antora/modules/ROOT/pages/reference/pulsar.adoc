...
Spring Boot auto-configuration also provides this consumer factory which you can further configure by specifying most of the {spring-boot-pulsar-config-props}[`spring.pulsar.consumer.*`] application properties (`subscription-name` is ignored and is instead generated when not specified on the annotation).
...
----
spring.pulsar.consumer.topics: hello-pulsar-topic
----
