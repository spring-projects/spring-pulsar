...
Keep in mind that, for end-user applications, in most scenarios, we recommend using `PulsarListener` annotation directly for consuming from a Pulsar topic when using Spring for Apache Pulsar, as that model covers a wide swath of application use cases.
However, it is important to understand how `PulsarListener` works internally and this section will go through those deatis.
...
If the topic is over partitioned, then messages will be load balanced across the consumers that the container creates.
...
When using the `Failover` subscription this way on partitioned topics, Pulsar guarantees message ordering.
