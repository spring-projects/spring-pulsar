...
The container provided pause and resume features sit a level atop of the similar capabilities provided by the Pulsar consumer which means that in addition to pausing and resuming the Pulsar consumer, the container will not even invoke the `receive` method on the consumer while it is paused.
...
Once the application gets a handle on this bean, then it can query for the listener container by its id and then invoke pause and resume operations.
...
When running this app, you will see the following output:
