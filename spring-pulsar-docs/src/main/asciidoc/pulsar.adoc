...
The following illustrates the steps to configure your Spring Boot app to use Zipkin with Brave. Note that the Spring Pulsar samples apps are configured the same way.

Add these dependencies to your application (Maven or Gradle, respectively):
...
.Add these properties to your application:
[source,yaml,indent=0,subs="verbatim"]
----
management:
...
----
NOTE: The `tracing.endpoint` above expects Zipkin is running locally as described https://zipkin.io/pages/quickstart.html[here].

At this point, your application should be recording traces when you send and receive Pulsar messages. You should be able to view them in the http://localhost:9411[Zipkin UI].

The above configuration can also be seen on the  link:{github}/blob/main/spring-pulsar-sample-apps/README.adoc[Spring Pulsar Sample Apps].
