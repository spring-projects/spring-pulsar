...
Let us take a few more ways to consume single records using `PulsarListener` annotation.
...
public void listen3(org.apache.pulsar.client.api.Message<String> message) {
...
----
@PulsarListener(subscriptionName = "my-subscription-2", topics = "my-topic-2", schemaType = SchemaType.JSON)
public void listen3(org.springframework.messaging.Message<String> message) {
...
----
@PulsarListener(subscriptionName = "hello-batch-subscription", topics = "hello-batch", schemaType = SchemaType.JSON, batch = true)
public void listen4(List<org.springframework.messaging.Message<Foo>> messages) {
    System.out.println("records received :" + messages.size());
...
In this section, we will see how we can use the message metadata that Pulsar provides in its Message as Pulsar headers in a Spring application.
...
In this example, we are consuming the data as `String`.
In addition, we are also extracting the message meatadata as Spring headers using the `Header` annotation from the Spring Framework.
We just randomly picked the headers for message id and raw data.
You can also extract custom message properties set when publishing using the `Header` annotation as observed in the retrieval of the property `foo` in the above example code.

Now let us try to receive this same data, but with the envelope of a Pulsar `Message`.

====
[source,java]
----
@PulsarListener(topics = "pulsarMessageListenerWithHeaders")
void pulsarMessageListenerWithHeaders(org.apache.pulsar.client.api.Message<String> data, @Header(PulsarHeaders.MESSAGE_ID) MessageId messageId,
                @Header(PulsarHeaders.RAW_DATA) byte[] rawData,
                @Header("foo") String foo) {
...
In this case, the user can directly call the corresponding methods on the Pulsar Message for retrieving the metadata.
However, as a convenience, you can also retrieve it using the `Header` annotation.

Here is another example in which we use the Spring messaging `Message` envelope.

====
[source,java]
----
@PulsarListener(topics = "springMessagingMessageListenerWithHeaders")
void springMessagingMessageListenerWithHeaders(org.springframework.messging.Message<String> data, @Header(PulsarHeaders.MESSAGE_ID) MessageId messageId,
                @Header(PulsarHeaders.RAW_DATA) byte[] rawData,
                @Header("foo") String foo) {
...
In this example, we are consuming the data as `List<String>`.
...
Here are two more examples of consuming in batch mode and extracting the headers using both Pulsar Message and Spring Message.

====
[source,java]
----
@PulsarListener(topics = "pulsarMessageBatchListenerWithHeaders", batch = true)
void pulsarMessageBatchListenerWithHeaders(List<org.apache.pulsar.client.api.Message<String>> data,
					@Header(PulsarHeaders.MESSAGE_ID) List<MessageId> messageIds,
					@Header(PulsarHeaders.TOPIC_NAME) List<String> topicNames, @Header("foo") List<String> fooValues) {
...
[source,java]
----
@PulsarListener(topics = "springMessagingMessageBatchListenerWithHeaders", batch = true)
